...
export default function JobQueueMenu() {
  ...
  return (
    <div className="flex flex-row items-center justify-center w-full h-full space-x-4">
      <button
        className={`px-4 py-2 text-white rounded hover:bg-blue-600 w-1/2 sm:w-1/2 lg:w-1/4 xl:w-1/6 ${jobQueueSelected === JobQueueEnum.Unrated ? 'bg-blue-600' : 'bg-blue-500 cursor-pointer'}`}
      ...
      <button
        className={`px-4 py-2 text-white rounded hover:bg-blue-600 w-1/2 sm:w-1/2 lg:w-1/4 xl:w-1/6 ${jobQueueSelected === JobQueueEnum.Liked ? 'bg-blue-600' : 'bg-blue-500 cursor-pointer'}`}
